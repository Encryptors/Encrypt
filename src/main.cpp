...
#if defined(NDEBUG)
# error "Encryptcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x11c0d373943b3a95b26e10553c5241c9d21dc4140bc369242c8538716f863749");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Encryptcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Encryptcoin Signed Message:\n";
...
CCCoinsViewMemPool::CCoinsViewMemPool(CCoinsView &baseIn, CTxMemPool &mempoolIn) : CCoinsViewBacked(baseIn), mempool(mempoolIn) { }
...
bool AddOrphanTxs(const CTransaction& tx)
{
...
{
    // Encryptcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CTxMemPool:accepts(CValidationState &state, CTransaction &tx, bool fCheckInputs, bool fLimitFree,
...
{
    int64 nSubsidy = 475 * COIN;
	
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60; // Encryptcoin: 1 minute
static const int64 nTargetSpacing = 1 * 60; // Encryptcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Encryptcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Encryptcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x11c0d373943b3a95b26e10553c5241c9d21dc4140bc369242c8538716f863749");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNBC 08/Jun/2016 The Indian start-up that may sideswipe Apple Pay";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044921610fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1465383802;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2578459;
        ...
        assert(block.hashMerkleRoot == uint256("0x8af72e8442afa24a3008e7fd18a9311e6df83f802671287c57929ef0bcf4e464"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc6, 0xb4, 0xc6 }; // Encryptcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWorks(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EncryptcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EncryptcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EncrypcoinMiner(CWallet *pwallet)
{
    printf("EncryptcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("encryptcoin-miner");
    ...
    try { loop {
        ...
        printf("Running EncryptcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EncryptcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EncryptcoinMiner, pwallet));
}
